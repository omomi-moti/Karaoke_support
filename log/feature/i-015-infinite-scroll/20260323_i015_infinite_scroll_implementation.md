...
**日付**: 2026-03-23  
**最終更新**: 2026-03-22（コード再検証・本ドキュメント整合／削除×追加読み込みレースのメモ追記）  
...
- **並び替え（点数・日付昇順）とページング**: Repository は常に `performedAt` 降順でページング。UI の `sortOrder` は **読み込み済み行の範囲**に対してのみ意味が揃う（I-014-B の注記どおり、DB 側 sort との整理は将来検討可）。
