...
- **Sources/Data/Network/NetworkMonitor.swift**
  - **NetworkMonitor**（`@MainActor` + `@Observable` な final class）
    ...
    - `pathUpdateHandler` 内で `Task { @MainActor in }` により `isOnline` をメインスレッドで更新する。queue に依存せず Swift 6 厳格並行性に対応。
...
- **Sources/App/Environment/NetworkMonitorEnvironment.swift**
...
| **App 層** | `Sources/App/Environment/NetworkMonitorEnvironment.swift` で EnvironmentKey を定義。`KaraokeSupportApp` で NetworkMonitor を生成・保持し、ルート View に注入。 |
...
- **スレッド**: `pathUpdateHandler` 内で `Task { @MainActor in }` により `isOnline` をメインスレッドで更新する。queue に依存せず Swift 6 厳格並行性に対応。
...
| pathUpdateHandler 内で Task { @MainActor in } により isOnline をメインスレッドで更新 | `Task { @MainActor [weak self] in self?.isOnline = ... }` で実装。queue に依存せず Swift 6 厳格並行性対応 | ✅ |

**致命的バグの有無**: なし。`Task { @MainActor in }` によるメインスレッドでの `isOnline` 更新、`deinit` での `cancel`、本番で監視 1 本・default で 0 本であることを確認済み。
