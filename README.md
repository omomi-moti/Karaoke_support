...
- **スコープ**: **V1** は手動曲名入力でコア体験が完結。**V2** は Spotify 連携・検索など体験拡張（設計済み・一部未実装）。
...
ユーザーインタビューやカラオケ体験の分析から、一人カラオケの動機は大きく「発散」「感情」「上達」の 3 軸に集約されることがわかった。選択肢が多すぎると記録時の認知負荷が上がり、少なすぎると文脈が残らない。**3 択なら 1 秒で選べて、かつ振り返り時に意味のある軸になる**バランスを狙った。
