...
| 層 | 読む場所 |
|----|----------|
| **短く** | 上の要約 → 「技術スタック」→「クイックスタート」までで全体像 |
...
|------|------------|------------------------|
| **UserDefaults** | キー・バリュー向け。リレーション・クエリに不向き。モデル単位の型安全も弱い。 | 不採用 |
| **FileManager** | 生ファイル運用。整合性・検索・リレーションを自前で担保する必要があり、不具合が入り込みやすい。 | 不採用 |
| **Core Data** | 表現力は十分。`NSManagedObject`・スタック構成・マイグレーションの手間が大きく、V1 の学習・実装コストとのバランスを考えると重い。 | 今回は見送り |
| **SwiftData** | `@Model` で Swift ネイティブに近い形でスキーマを定義でき、**モデル境界・コンテキスト経由のアクセス** により永続化の安全性を確保しやすい。iOS 17+ 前提のうえ **導入・変更のしやすさ** も両立しやすい。 | **採用** |

#### `@Observable`（iOS 17+・ViewModel の状態管理）

ViewModel は **`@Observable`** マクロ付きのクラスとして定義する。従来の `ObservableObject` ベースと比較すると次のとおり。

| 観点 | `ObservableObject` + `@Published` | `@Observable` |
|------|-------------------------------------|----------------|
| 変更通知 | `@Published` や `objectWillChange` の記述が増えやすい | マクロが **プロパティ変更を観測** |
| View との接続 | `@StateObject` / `@ObservedObject` で **所有・ライフサイクル**を整理 | `@State` / `@Bindable` で保持・バインドしやすい |
| ViewModel の書き方 | `ObservableObject` 準拠が前提 | **通常のクラス**として記述しやすい |

**採用理由**: `@Published` / `objectWillChange` のボイラープレート削減と、View 側の **所有の説明の単純化**を優先した。
...
| **手動 DI（`@Environment`）** | Swinject 等のコンテナは依存関係が増え、登録・解決のルールが複雑になるほど効果が出る。V1 は Repository 等の依存グラフが浅く、注入先も限定的なため、**ライブラリ依存と登録ボイラープレートを増やす割に得られる整理の幅が小さい**。SwiftUI の `Environment` でプロトコル型を渡す方式なら、**公式の受け渡しパターンと一致**し、プレビュー・テストでの差し替えもそのまま行える。 |
...
本節は **このアプリでの配線・運用ルール（How）** — 画面層のラッパー分担、データの置き場、非同期・オフラインの扱い — をまとめる。**SwiftData / `@Observable` / Repository / 手動 DI をなぜ採用したか（Why・他手段との比較）** は、[技術選定の理由](#技術選定の理由) に集約しており、ここでは同じ比較を繰り返さない。
...
flowchart LR
    ...
    classDef nodeSpec fill:#16213e,stroke:#0f3460,color:#fff
    classDef nodeMilestone fill:#1a5c2e,stroke:#2ecc71,color:#fff
    class R,S,BD,ISS,CC,BR nodeSpec
    class LG nodeMilestone
```

上図は主要な作業フロー（メインストリーム）を示しています。

- **基本フロー**: 上図の順序に従って進行します。
- **例外処理（修正ループ）**: `cross_check_report` の段階で仕様書（spec）・設計書・Issue 間に矛盾が確認された場合のみ、該当ドキュメントを修正します。
- **補足**: 設計と Issue の間を恒常的に行き来するような運用ではありません。

**フロー内の補足（各ステップの詳細）** — 上図の ①〜⑦ に対応。

- **① 要求仕様書** — `docs/raw_spec.md`（v4.1 系）に課題・機能要求・制約（Spotify 規約など）を記述する。
- **② SpecKit で Feature Specification（spec.md）を生成**
  - User Story / Acceptance Scenario / Functional Requirements を定義
- **③ 基本設計書（basic_design.md）・詳細設計書（detailed_design.md）を作成**
  - 画面遷移図・シーケンス図・クラス図・ER 図を Mermaid で記述
- **④ Issue 設計（`docs/issues.md` → `docs/v1_issues.md`（V1 の正））で実装タスクに分解**
...
- **⑤ ドキュメント棚卸し** — `docs/cross_check_report.md` で spec・設計書・issues の整合性を確認する。
- **⑥ 実装** — 1 Issue 1 **feature ブランチ**で作業し、Pull Request 経由でレビュー・**squash merge** する。
- **⑦ 実装ログ** — `log/feature/i-xxx-*/` に、Issue 単位の実装メモ・判断の記録を残す。
...
[`docs/raw_spec.md`](docs/raw_spec.md) v4.1、[Feature Specification（`specs/001-hitora-karaoke-ios/spec.md`）](specs/001-hitora-karaoke-ios/spec.md)、[`docs/basic_design.md`](docs/basic_design.md)、[`docs/detailed_design.md`](docs/detailed_design.md) に、**プロダクト全体**の要求・設計をまとめている。**V1（MVP）の範囲・完了条件の正**は常に [`docs/v1_issues.md`](docs/v1_issues.md) とする。


| 見たいもの | 参照先 |
|------------|--------|
| V1 のタスク・完了チェック | [`docs/v1_issues.md`](docs/v1_issues.md) |
| 全フェーズ・将来機能（ロードマップ） | [`docs/issues.md`](docs/issues.md)、および下記「未実装 / 今後の実装」節 |
| 選曲〜記録の画面遷移・V1 での意図 | [`docs/v1_navigation_songs_recording.md`](docs/v1_navigation_songs_recording.md) |
| ドキュメント間の整合性の棚卸し（履歴） | [`docs/cross_check_report.md`](docs/cross_check_report.md) |
| `fetchByIntent` など SwiftData 前提の実装判断 | README「技術的な工夫」「技術的な壁と解決策」 |

V1 のゴールは **記録 → 振り返りのコア体験が成立すること**。Spotify 連携などの拡張は Issue 上の Phase 3/4 で追う。
...
> V1 は手動曲名入力でコア体験が完結する。**Spotify 連携は V2 以降の予定**のため、Spotify Developer Dashboard の設定は不要。
