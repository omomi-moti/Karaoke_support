...
- **Tasks**:
  ...
  - [ ] フォルダ構成の雛形を作成する（Views, Models, Repositories, Services等）
...
- **Tasks**:
  ...
  - [ ] SingingSession エンティティを @Model で定義する（id, track, intent, performedAt, score, memo, spotifyTrackId）
...
- **Labels**: `priority:must`, `type:feat`, `phase:1-MVP`
- **Tasks**:
  - [ ] fetchAll(limit, offset) を用いたページネーションを実装する
...
### [I-039] UIテスト（主要フロー）
- **Labels**: `priority:could`, `type:chore`, `phase:4-拡張`
- **Tasks**:
  - [ ] 歌唱記録フロー（曲選択→Intent→入力→保存）の E2E テストを実装する
  - [ ] 履歴閲覧フローの E2E テストを実装する
  - [ ] XCUITest で UI 要素を特定し、操作をシミュレートする
  - [ ] テスト用のモックまたはスタブを用意する（必要に応じて）
