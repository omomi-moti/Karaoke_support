...
- **Tasks**:
  ...
  - [x] fetchAll(limit, offset) を実装する（日時降順）。offset はスキップ件数（0-based）
...
- **Tasks**:
  ...
  - [x] getOrCreate(spotifyTrackId?, userEnteredName?) を実装する（既存検索 or 新規作成）
...
- **拡張（詳細タスク）**: 色・テーマ **I-014-A**、ソート **I-014-B**、履歴からの更新 **I-014-C** は [`v1_issues.md`](./v1_issues.md) の I-014 分岐を正とする。
