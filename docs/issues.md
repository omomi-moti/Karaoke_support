...
| I-007A | 依存性注入（DI）接続 | Must | @Environment + EnvironmentKey で Repository を注入。ViewModel が初期化時に受け取る |
...
Phase 1:
  I-007 → I-007A → I-008, I-009, I-010, I-011（並列可能）
  I-012（I-007と並列可能）
  I-013（I-007A, I-008, I-009, I-010, I-011, I-012 完了後）
  I-014 → I-015
  I-016（I-007 完了後）
...
- **I-013**: I-003, I-004, I-007A, I-008, I-009, I-010, I-011, I-012 に依存。これらは I-013 に依存しない。✓
...
実装着手後に最初に検証する項目。実現可能性が不明な投機的設計を排除するため、小さなプロトタイプで結果を確認した後に仕様にフィードバックする。

| # | 検証内容 | 完了条件 | 対応 Issue |
|---|---------|---------|----------|
| Spike-1 | SwiftData `FetchDescriptor` によるIntent別集計クエリ（`group by`相当）が1000件超で体感0.5秒以内に返るか | InstrumentsのタイムプロファイラでView Bodyの長時間実行が検出されないこと | I-005 |
| Spike-2 | `actor TrackMetadataCache` の`get`/`set`が複数の非同期コンテキストから並行アクセスした際にクラッシュ・データ競合しないか | XCTestの`async`テストで複数タスクから同時アクセスしてエラーが発生しないことを確認 | I-024A |
