...
Spotify API 規約により、メタデータの永続保存が禁止されている。UserDefaults への永続保存は規約違反の可能性がある。24 時間以内の一時キャッシュ（インメモリまたは有効期限付きの一時ストレージ）のみを使用する。アプリ起動時・手動リフレッシュで API から再取得し、キャッシュを上書きする。
