...
**指摘（履歴）**: 当初は、Spotify API 規約によりメタデータの永続保存が禁止されているにもかかわらず、UserDefaults に永続化するよう読める記述があり、24時間以内の一時キャッシュ（インメモリまたは有効期限付き UserDefaults）という方針と矛盾していた。  
**対応状況**: 本PRで issues.md I-025, basic_design.md 2.1, raw_spec.md 5.3, detailed_design.md 3.4 を「24時間以内の一時キャッシュ」「永続化しない」方針に揃えて修正済み。UserDefaults を用いる場合も有効期限付きの一時キャッシュのみを想定しており、メタデータを永続保存しない設計となっている。
